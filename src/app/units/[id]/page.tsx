...
import { ParkingUnitPage } from "@/components/units/parking-unit-page";
...
import { PublicApiError, fetchAllPautaliaUnits, fetchPautaliaBuilding, fetchPautaliaParkingUnit, fetchPautaliaUnit } from "@/lib/public-api";
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (error instanceof PublicApiError && error.status === 404) {
      try {
        const parking = await fetchPautaliaParkingUnit(locale, id);
        return buildPageMetadata({
          locale,
          pathname: `/units/${parking.slug}`,
          title: locale === "bg" ? `Паркомясто ${parking.code}` : `Parking ${parking.code}`,
          description:
            locale === "bg"
              ? `${parking.code} в ${getBuildingLabel(locale, parking.buildingId, userType).toLowerCase()} с площ ${parking.size} кв.м.`
              : `${parking.code} in building ${parking.buildingId.toUpperCase()} parking inventory.`,
          imagePath: "/assets/buildings/residence/gallery/living-entry.jpg",
          imageAlt: locale === "bg" ? `Паркомясто ${parking.code}` : `Parking ${parking.code}`,
        });
      } catch (parkingError) {
        if (parkingError instanceof PublicApiError && parkingError.status === 404) {
...
export default async function UnitPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (error instanceof PublicApiError && error.status === 404) {
      try {
        const parking = await fetchPautaliaParkingUnit(locale, id);
        return <ParkingUnitPage unit={parking} locale={locale} userType={userType} />;
      } catch (parkingError) {
        if (parkingError instanceof PublicApiError && parkingError.status === 404) {
          notFound();
        }
  ...
  return (
    <>
      <Script id={`unit-jsonld-${unit.id}`} type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(getUnitJsonLd(unit, locale)) }} />
