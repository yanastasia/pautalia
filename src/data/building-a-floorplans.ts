import type { Floor, Unit, UnitPlanArea } from "@/types/domain";

export const buildingAFacadePhoto = "/assets/buildings/residence/selector/main.png";

export const buildingAFloorPlanImages = {
  1: "/assets/buildings/residence/floors/floor-01.png",
  2: "/assets/buildings/residence/floors/floor-02.png",
  3: "/assets/buildings/residence/floors/floor-03.png",
  4: "/assets/buildings/residence/floors/floor-04.png",
} as const;

const buildingAUnitFloorplanImages = {
  "A-AP.01": "/assets/buildings/residence/apartments/A-AP.01.png",
  "A-AP.02": "/assets/buildings/residence/apartments/A-AP.02.png",
  "A-AP.03": "/assets/buildings/residence/apartments/A-AP.03.png",
  "A-AP.04": "/assets/buildings/residence/apartments/A-AP.04.png",
  "A-AP.05": "/assets/buildings/residence/apartments/A-AP.05.png",
  "A-AP.06": "/assets/buildings/residence/apartments/A-AP.06.png",
  "A-AP.07": "/assets/buildings/residence/apartments/A-AP.07.png",
  "A-AP.08": "/assets/buildings/residence/apartments/A-AP.08.png",
  "A-AP.09": "/assets/buildings/residence/apartments/A-AP.09.png",
  "A-AP.10": "/assets/buildings/residence/apartments/A-AP.10.png",
  "A-AP.11": "/assets/buildings/residence/apartments/A-AP.11.png",
  "A-AP.12": "/assets/buildings/residence/apartments/A-AP.12.png",
  "A-AP.13": "/assets/buildings/residence/apartments/A-AP.13.png",
  "A-AP.14": "/assets/buildings/residence/apartments/A-AP.14.png",
} as const;

const buildingAUnitPolygonPoints = {
  "A-AP.01": ["12.5,30.2 26.5,30.2 26.5,26.3 37.5,26.3 37.5,41.2 27.5,41.2 27.5,70.25 12.5,70.25"],
  "A-AP.02": ["26.75,40.5 49.5,40.5 49.5,70.25 26.75,70.25"],
  "A-AP.03": ["48.8,41.4 71.57,41.4 71.57,70.25 48.8,70.25"],
  "A-AP.04": ["71.9,31.07 88.15,31.07 88.15,70.25 70.85,70.25 70.85,42.1 60.8,42.1 60.8,27.15 71.9,27.15"],

  "A-AP.05": ["0,20.8 6,20.8 6,11.5 28,11.5 28,42 8,42 8,86 0,86 0,20.8"],
  "A-AP.06": ["6.7,40.5 50.3,40.5 50.3,88 6.7,88"],
  "A-AP.07": ["49,42 93,42 93,90 49,90"],
  "A-AP.08": ["100,22 93.9,22 93.9,12.9 72,12.9 72,44 92,44 92,89 100,89 100,22"],

  "A-AP.09": ["0,20.8 6,20.8 6,11.5 28,11.5 28,42 8,42 8,86 0,86 0,20.8"],
  "A-AP.10": ["6.7,40.5 50.3,40.5 50.3,88 6.7,88"],
  "A-AP.11": ["49,42 93,42 93,90 49,90"],
  "A-AP.12": ["100,22 93.9,22 93.9,12.9 72,12.9 72,44 92,44 92,89 100,89 100,22"],

  "A-AP.13": ["6,25.5 28.5,25.5 28.5,41 50.7,41 50.7,86.8 0,86.8 0,25.5 6,25.5"],
  "A-AP.14": ["90,27 71.5,27 71.5,42.5 49.3,42.5 49.3,88.5 100,88.5 100,27 90,27"]
} as const;

export const buildingAFloorPlanMediaIds = {
  1: "media-floorplan-a-1",
  2: "media-floorplan-a-2",
  3: "media-floorplan-a-3",
  4: "media-floorplan-a-4",
} as const;

type BuildingAFloorNumber = keyof typeof buildingAFloorPlanImages;

export const buildingAFloorOverrides: Record<string, Partial<Floor>> = Object.fromEntries(
  Object.entries(buildingAFloorPlanImages).map(([floor, image]) => [
    `a-${floor}`,
    {
      floorplanImage: image,
      floorplanImageId: buildingAFloorPlanMediaIds[Number(floor) as keyof typeof buildingAFloorPlanMediaIds],
      mapAspectRatio: "1 / 1",
    },
  ]),
);

export type BuildingAParsedUnit = Pick<
  Unit,
  | "id"
  | "slug"
  | "externalCode"
  | "code"
  | "unitNumber"
  | "floor"
  | "typologyId"
  | "rooms"
  | "bedrooms"
  | "bathrooms"
  | "areaInternalSqm"
  | "areaTotalSqm"
  | "terraceSqm"
  | "hasYard"
  | "outdoorType"
  | "size"
  | "orientation"
  | "exposure"
  | "highlight"
  | "description"
  | "floorplan"
  | "floorplanImageId"
  | "features"
  | "planArea"
  | "planRegions"
  | "planPolygonPoints"
  | "seoTitle"
  | "seoDescription"
>;

function createParsedUnit({
  floor,
  planArea,
  ...unit
}: Omit<BuildingAParsedUnit, "floorplan" | "floorplanImageId" | "planArea" | "floor"> & {
  floor: BuildingAFloorNumber;
  planArea: UnitPlanArea;
}): BuildingAParsedUnit {
  return {
    ...unit,
    floor,
    floorplan: buildingAUnitFloorplanImages[unit.code as keyof typeof buildingAUnitFloorplanImages] ?? buildingAFloorPlanImages[floor],
    floorplanImageId: buildingAFloorPlanMediaIds[floor],
    planArea,
    planPolygonPoints: buildingAUnitPolygonPoints[unit.code as keyof typeof buildingAUnitPolygonPoints],
  };
}

export const buildingAParsedUnits: BuildingAParsedUnit[] = [
  createParsedUnit({
    id: "a-101",
    slug: "unit-a101",
    externalCode: "A-AP.01",
    code: "A-AP.01",
    unitNumber: "01",
    floor: 1,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 66.23,
    areaTotalSqm: 66.23,
    terraceSqm: 0,
    hasYard: true,
    outdoorType: "yard",
    size: 66.23,
    orientation: "south-east",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a private yard.",
    description: "Ground-floor three-room apartment with two bedrooms, one bathroom, and a private yard.",
    features: ["Two bedrooms", "One bathroom", "Private yard"],
    planArea: { x: 74.8, y: 33, width: 24, height: 40 },
    planRegions: [
      { x: 74.8, y: 33, width: 24.2, height: 24.6 },
      { x: 62.4, y: 56.1, width: 36.6, height: 18.8 },
    ],
    seoTitle: "Residence A-AP.01 | Three-room apartment",
    seoDescription: "Ground-floor three-room apartment with two bedrooms, one bathroom, and a private yard.",
  }),
  createParsedUnit({
    id: "a-102",
    slug: "unit-a102",
    externalCode: "A-AP.02",
    code: "A-AP.02",
    unitNumber: "02",
    floor: 1,
    typologyId: "typology-2b",
    rooms: 2,
    bedrooms: 1,
    bathrooms: 1,
    areaInternalSqm: 52.89,
    areaTotalSqm: 52.89,
    terraceSqm: 0,
    hasYard: true,
    outdoorType: "yard",
    size: 52.89,
    orientation: "south",
    exposure: "single-aspect",
    highlight: "Two-room apartment with one bedroom and a private yard.",
    description: "Ground-floor two-room apartment with one bedroom, one bathroom, and a private yard.",
    features: ["One bedroom", "One bathroom", "Private yard"],
    planArea: { x: 60.5, y: 33.5, width: 23, height: 23 },
    seoTitle: "Residence A-AP.02 | Two-room apartment",
    seoDescription: "Ground-floor two-room apartment with one bedroom, one bathroom, and a private yard.",
  }),
  createParsedUnit({
    id: "a-103",
    slug: "unit-a103",
    externalCode: "A-AP.03",
    code: "A-AP.03",
    unitNumber: "03",
    floor: 1,
    typologyId: "typology-2b",
    rooms: 2,
    bedrooms: 1,
    bathrooms: 1,
    areaInternalSqm: 52.82,
    areaTotalSqm: 52.82,
    terraceSqm: 0,
    hasYard: true,
    outdoorType: "yard",
    size: 52.82,
    orientation: "south",
    exposure: "single-aspect",
    highlight: "Two-room apartment with one bedroom and a private yard.",
    description: "Ground-floor two-room apartment with one bedroom, one bathroom, and a private yard.",
    features: ["One bedroom", "One bathroom", "Private yard"],
    planArea: { x: 34.5, y: 32.5, width: 24, height: 23 },
    seoTitle: "Residence A-AP.03 | Two-room apartment",
    seoDescription: "Ground-floor two-room apartment with one bedroom, one bathroom, and a private yard.",
  }),
  createParsedUnit({
    id: "a-104",
    slug: "unit-a104",
    externalCode: "A-AP.04",
    code: "A-AP.04",
    unitNumber: "04",
    floor: 1,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 66.23,
    areaTotalSqm: 66.23,
    terraceSqm: 0,
    hasYard: true,
    outdoorType: "yard",
    size: 66.23,
    orientation: "south-west",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a private yard.",
    description: "Ground-floor three-room apartment with two bedrooms, one bathroom, and a private yard.",
    features: ["Two bedrooms", "One bathroom", "Private yard"],
    planArea: { x: 17.5, y: 32.2, width: 27.8, height: 41.2 },
    planRegions: [
      { x: 17.5, y: 32.2, width: 16.8, height: 18.8 },
      { x: 17.5, y: 47.0, width: 27.6, height: 16.8 },
      { x: 28.8, y: 63.4, width: 16.3, height: 9.8 },
    ],
    seoTitle: "Residence A-AP.04 | Three-room apartment",
    seoDescription: "Ground-floor three-room apartment with two bedrooms, one bathroom, and a private yard.",
  }),
  createParsedUnit({
    id: "a-201",
    slug: "unit-a201",
    externalCode: "A-AP.05",
    code: "A-AP.05",
    unitNumber: "05",
    floor: 2,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 80.86,
    areaTotalSqm: 80.86,
    terraceSqm: 4,
    hasYard: false,
    outdoorType: "balcony",
    size: 80.86,
    orientation: "south-east",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a balcony.",
    description: "Second-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
    features: ["Two bedrooms", "One bathroom", "Separate WC", "Balcony"],
    planArea: { x: 74.43, y: 28.89, width: 25.45, height: 46.8 },
    planRegions: [
      { x: 74.43, y: 28.89, width: 25.45, height: 16.9 },
      { x: 74.43, y: 42.1, width: 16.8, height: 24.8 },
      { x: 82.55, y: 46.2, width: 17.33, height: 20.5 },
    ],
    seoTitle: "Residence A-AP.05 | Three-room apartment",
    seoDescription: "Second-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
  }),
  createParsedUnit({
    id: "a-202",
    slug: "unit-a202",
    externalCode: "A-AP.06",
    code: "A-AP.06",
    unitNumber: "06",
    floor: 2,
    typologyId: "typology-2b",
    rooms: 2,
    bedrooms: 1,
    bathrooms: 1,
    areaInternalSqm: 63.51,
    areaTotalSqm: 63.51,
    terraceSqm: 4,
    hasYard: false,
    outdoorType: "balcony",
    size: 63.51,
    orientation: "south",
    exposure: "single-aspect",
    highlight: "Two-room apartment with one bedroom and a balcony.",
    description: "Second-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
    features: ["One bedroom", "One bathroom", "Balcony"],
    planArea: { x: 60.31, y: 27.06, width: 24.85, height: 29.54 },
    seoTitle: "Residence A-AP.06 | Two-room apartment",
    seoDescription: "Second-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
  }),
  createParsedUnit({
    id: "a-203",
    slug: "unit-a203",
    externalCode: "A-AP.07",
    code: "A-AP.07",
    unitNumber: "07",
    floor: 2,
    typologyId: "typology-2b",
    rooms: 2,
    bedrooms: 1,
    bathrooms: 1,
    areaInternalSqm: 63.44,
    areaTotalSqm: 63.44,
    terraceSqm: 4,
    hasYard: false,
    outdoorType: "balcony",
    size: 63.44,
    orientation: "south",
    exposure: "single-aspect",
    highlight: "Two-room apartment with one bedroom and a balcony.",
    description: "Second-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
    features: ["One bedroom", "One bathroom", "Balcony"],
    planArea: { x: 34.14, y: 25.88, width: 24.85, height: 29.67 },
    seoTitle: "Residence A-AP.07 | Two-room apartment",
    seoDescription: "Second-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
  }),
  createParsedUnit({
    id: "a-204",
    slug: "unit-a204",
    externalCode: "A-AP.08",
    code: "A-AP.08",
    unitNumber: "08",
    floor: 2,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 80.86,
    areaTotalSqm: 80.86,
    terraceSqm: 4.26,
    hasYard: false,
    outdoorType: "balcony",
    size: 80.86,
    orientation: "south-west",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a balcony.",
    description: "Second-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
    features: ["Two bedrooms", "One bathroom", "Separate WC", "Balcony"],
    planArea: { x: 14.11, y: 27.71, width: 30.76, height: 46.8 },
    planRegions: [
      { x: 14.11, y: 27.71, width: 30.76, height: 17.1 },
      { x: 14.11, y: 46.0, width: 17.4, height: 21.0 },
      { x: 28.7, y: 41.9, width: 16.17, height: 24.9 },
    ],
    seoTitle: "Residence A-AP.08 | Three-room apartment",
    seoDescription: "Second-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
  }),
  createParsedUnit({
    id: "a-301",
    slug: "unit-a301",
    externalCode: "A-AP.09",
    code: "A-AP.09",
    unitNumber: "09",
    floor: 3,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 80.86,
    areaTotalSqm: 80.86,
    terraceSqm: 4,
    hasYard: false,
    outdoorType: "balcony",
    size: 80.86,
    orientation: "south-east",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a balcony.",
    description: "Third-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
    features: ["Two bedrooms", "One bathroom", "Separate WC", "Balcony"],
    planArea: { x: 74.43, y: 28.89, width: 25.45, height: 46.8 },
    planRegions: [
      { x: 74.43, y: 28.89, width: 25.45, height: 16.9 },
      { x: 74.43, y: 42.1, width: 16.8, height: 24.8 },
      { x: 82.55, y: 46.2, width: 17.33, height: 20.5 },
    ],
    seoTitle: "Residence A-AP.09 | Three-room apartment",
    seoDescription: "Third-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
  }),
  createParsedUnit({
    id: "a-302",
    slug: "unit-a302",
    externalCode: "A-AP.10",
    code: "A-AP.10",
    unitNumber: "10",
    floor: 3,
    typologyId: "typology-2b",
    rooms: 2,
    bedrooms: 1,
    bathrooms: 1,
    areaInternalSqm: 63.51,
    areaTotalSqm: 63.51,
    terraceSqm: 4,
    hasYard: false,
    outdoorType: "balcony",
    size: 63.51,
    orientation: "south",
    exposure: "single-aspect",
    highlight: "Two-room apartment with one bedroom and a balcony.",
    description: "Third-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
    features: ["One bedroom", "One bathroom", "Balcony"],
    planArea: { x: 60.31, y: 27.06, width: 24.85, height: 29.54 },
    seoTitle: "Residence A-AP.10 | Two-room apartment",
    seoDescription: "Third-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
  }),
  createParsedUnit({
    id: "a-303",
    slug: "unit-a303",
    externalCode: "A-AP.11",
    code: "A-AP.11",
    unitNumber: "11",
    floor: 3,
    typologyId: "typology-2b",
    rooms: 2,
    bedrooms: 1,
    bathrooms: 1,
    areaInternalSqm: 63.44,
    areaTotalSqm: 63.44,
    terraceSqm: 4,
    hasYard: false,
    outdoorType: "balcony",
    size: 63.44,
    orientation: "south",
    exposure: "single-aspect",
    highlight: "Two-room apartment with one bedroom and a balcony.",
    description: "Third-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
    features: ["One bedroom", "One bathroom", "Balcony"],
    planArea: { x: 34.14, y: 25.88, width: 24.85, height: 29.67 },
    seoTitle: "Residence A-AP.11 | Two-room apartment",
    seoDescription: "Third-floor two-room apartment with one bedroom, one bathroom, and a balcony.",
  }),
  createParsedUnit({
    id: "a-304",
    slug: "unit-a304",
    externalCode: "A-AP.12",
    code: "A-AP.12",
    unitNumber: "12",
    floor: 3,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 80.86,
    areaTotalSqm: 80.86,
    terraceSqm: 3.43,
    hasYard: false,
    outdoorType: "balcony",
    size: 80.86,
    orientation: "south-west",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a balcony.",
    description: "Third-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
    features: ["Two bedrooms", "One bathroom", "Separate WC", "Balcony"],
    planArea: { x: 14.11, y: 27.71, width: 30.76, height: 46.8 },
    planRegions: [
      { x: 14.11, y: 27.71, width: 30.76, height: 17.1 },
      { x: 14.11, y: 46.0, width: 17.4, height: 21.0 },
      { x: 28.7, y: 41.9, width: 16.17, height: 24.9 },
    ],
    seoTitle: "Residence A-AP.12 | Three-room apartment",
    seoDescription: "Third-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a balcony.",
  }),
  createParsedUnit({
    id: "a-401",
    slug: "unit-a401",
    externalCode: "A-AP.13",
    code: "A-AP.13",
    unitNumber: "13",
    floor: 4,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 125.42,
    areaTotalSqm: 125.42,
    terraceSqm: 32.26,
    hasYard: false,
    outdoorType: "terrace",
    size: 125.42,
    orientation: "south-east",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a panoramic terrace.",
    description: "Fourth-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a panoramic terrace.",
    features: ["Two bedrooms", "One bathroom", "Separate WC", "Panoramic terrace"],
    planArea: { x: 61.4, y: 27.06, width: 38.48, height: 39.87 },
    planRegions: [
      { x: 61.4, y: 19.8, width: 38.48, height: 10.8 },
      { x: 92.1, y: 27.2, width: 7.78, height: 11.1 },
      { x: 61.4, y: 30.6, width: 38.48, height: 14.1 },
      { x: 61.4, y: 44.3, width: 27.2, height: 14.1 },
      { x: 85.4, y: 37.2, width: 14.48, height: 20.3 },
    ],
    seoTitle: "Residence A-AP.13 | Three-room apartment",
    seoDescription: "Fourth-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a panoramic terrace.",
  }),
  createParsedUnit({
    id: "a-402",
    slug: "unit-a402",
    externalCode: "A-AP.14",
    code: "A-AP.14",
    unitNumber: "14",
    floor: 4,
    typologyId: "typology-3a",
    rooms: 3,
    bedrooms: 2,
    bathrooms: 1,
    areaInternalSqm: 125.39,
    areaTotalSqm: 125.39,
    terraceSqm: 32.26,
    hasYard: false,
    outdoorType: "terrace",
    size: 125.39,
    orientation: "south-west",
    exposure: "dual-aspect",
    highlight: "Three-room apartment with two bedrooms and a panoramic terrace.",
    description: "Fourth-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a panoramic terrace.",
    features: ["Two bedrooms", "One bathroom", "Separate WC", "Panoramic terrace"],
    planArea: { x: 14.11, y: 25.88, width: 44.87, height: 40 },
    planRegions: [
      { x: 14.11, y: 19.8, width: 46.19, height: 10.8 },
      { x: 6.0, y: 27.2, width: 8.11, height: 11.2 },
      { x: 14.11, y: 30.8, width: 31.6, height: 27.8 },
      { x: 44.2, y: 30.8, width: 16.1, height: 13.7 },
      { x: 33.4, y: 41.5, width: 17.8, height: 10.1 },
    ],
    seoTitle: "Residence A-AP.14 | Three-room apartment",
    seoDescription: "Fourth-floor three-room apartment with two bedrooms, one bathroom, a separate WC, and a panoramic terrace.",
  }),
];
